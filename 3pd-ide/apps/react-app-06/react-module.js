...
(async () => {
  ...
  // ---------------------------------------------------------
  // 3. Failsafe: If user says "n" but no Drupal root is found, force 3PD mode
  // ---------------------------------------------------------
  const drupalWebRoot = findDrupalRoot(appRoot);

  if (!is3PD && !drupalWebRoot) {
    console.log(`
⚠️  You selected "internal developer" mode, but no Drupal web/ directory was found
...
`);
  ...
  if (!drupalWebRoot) {
    console.log(`
⚠️ INTERNAL MODE was requested, but no Drupal web/ root could be found
   by walking upward from this app folder.

   Falling back to 3PD-style behavior. The module has been generated locally at:
   ${moduleDir}
`);
    console.log("\n🎉 HUDX Drupal module created successfully (local only)!\n");
    return;
  }
  ...
  if (fs.existsExists && fs.existsSync(drupalModuleDir)) {
    fs.rmSync(drupalModuleDir, { recursive: true, force: true });
  } else if (fs.existsSync(drupalModuleDir)) {
    fs.rmSync(drupalModuleDir, { recursive: true, force: true });
  }

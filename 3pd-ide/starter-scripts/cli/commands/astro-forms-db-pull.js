 ...
 * File: commands/astro-forms-db-pull.js
 *
 ...
 *
 * Run from inside an astro-forms app folder:
 *   cd apps/<your-app>
 *   3pd astro-forms db pull
 *
 * What it does:
 *   1. Reads APP_SLUG and PUBLIC_DRUPAL_BASE_URL from .env
...
export default async function astroFormsDbPull({ ideRoot }) {
  const cwd     = process.cwd();
  const appName = path.basename(cwd);

  log.header('3PD Astro Forms — DB Pull');
  ...
  // -------------------------------------------------------------------------
  // Determine Drupal URL — .env first, then 3pd.config.json fallback
  // -------------------------------------------------------------------------
  let drupalUrl = env.PUBLIC_DRUPAL_BASE_URL;
  if (!drupalUrl) {
  ...
  if (!drupalUrl) {
    ...
    log.dim('Set PUBLIC_DRUPAL_BASE_URL in .env or pantheonUrl in 3pd.config.json');
    process.exit(1);
  }
  ...
  if (!fs.existsSync(schemaPath)) {
    log.error('server/db/schema.sql not found. Is this an astro-forms app?');
    process.exit(1);
  }
...
`.trim();
  ...
  log.dim('If dev server is running: Ctrl+C, then lsof -ti:3001 | xargs kill -9, then npm run dev');

 ...
 *
 * Generates a Drupal module from the current Fuse app.
 * Supports:
 *   - 3PD mode (default)
 ...
 */
export default async function fuseModule({ ideRoot, internal }) {
  log.header("Fuse Module Generator");
  ...
  if (!fs.existsSync(pkgPath)) {
    ...
    log.nl();
    log.info("Run this command from inside a Fuse app folder:");
    log.dim("  cd apps/<your-app>");
    log.dim("  3pd fuse module");
    log.nl();
    process.exit(1);
  }
  ...
  log.nl();
  log.success(`Fuse module generation complete for "${appName}".`);
  log.nl();
}

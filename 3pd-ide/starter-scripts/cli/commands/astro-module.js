/**
 * 3pd astro module
 * File: commands/astro-module.js
 *
 * Generates a Drupal module from the current Astro app.
 * Supports:
 *   - 3PD mode (default)
 ...
 */
export default async function astroModule({ ideRoot, internal }) {
  log.header('HUDX Astro Module Generator');
  ...
  if (!fs.existsSync(pkgPath)) {
    ...
    log.nl();
    log.info('Run this command from inside an Astro app folder:');
    log.dim('  cd apps/<your-app>');
    log.dim('  3pd astro module');
    log.nl();
    process.exit(1);
  }

    ...
    if ($info && $info['status'] !== 'not found') {
      return [
        ...
        'info' => [
          ...
          '#used_count' => isset($info['entities_limit']) ? min(intval($count), intval($info['entities_limit'])) : $count,
          '#limit' => $info['entities_limit'] ?? NULL,
        ...
        'explainer' => [
          '#markup' => $this->t('The content items limit is limiting how many pages, blocks, etc. you can edit with DXPR Builder. You can increase or eliminate the limit by <a href="https://dxpr.com/user/me/subscription/change">upgrading to a higher subscription tier</a>.'),
        ],
        'table' => [
          '#theme' => 'table',
          '#header' => $header,

...
/**
 * Handles POST /api/react---3pd-ai-helper/generate
...
- No random variations between requests';
    ...
    $db = \Drupal::database();
    $db->insert('hudx_react___3pd_ai_helper_history')
      ->fields([
        'title'      => $title ?: NULL,
        'prompt'     => $prompt,
        'markup'     => $markup,
    ...
    $id  = $db->query('SELECT MAX(id) FROM {hudx_react___3pd_ai_helper_history}')->fetchField();
    $row = $db->select('hudx_react___3pd_ai_helper_history', 'h')
      ->fields('h')
      ->condition('id', $id)
      ->execute()
      ->fetchAssoc();

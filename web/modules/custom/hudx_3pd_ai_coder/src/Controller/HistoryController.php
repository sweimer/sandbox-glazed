 ...
 *
 * GET    /api/3pd-ai-coder/history         — all rows newest first
 * PATCH  /api/3pd-ai-coder/history/{id}    — update node_url
 * DELETE /api/3pd-ai-coder/history/{id}    — delete a row
 */
class HistoryController extends ControllerBase {

  public function list(): JsonResponse {
    $rows = \Drupal::database()
      ->select('hudx_3pd_ai_coder_history', 'h')
      ->fields('h')
      ->orderBy('h.id', 'DESC')
      ->execute()
  ...
  public function update(Request $request, $id): JsonResponse {
    ...
    \Drupal::database()
      ->update('hudx_3pd_ai_coder_history')
  ...
  public function delete($id): Response {
    \Drupal::database()
      ->delete('hudx_3pd_ai_coder_history')
      ->condition('id', $id)
      ->execute();
